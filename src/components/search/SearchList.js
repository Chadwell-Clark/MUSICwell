...
import {getUsersArtists, searchArtists} from "../../modules/ArtistsManager.js";
...
export const SearchList = ({searchInputText}) => {
    ...
    if (searchResult.length > 0) {
     return (
        ...
        <div>{searchResult.map((artist) => <ArtistCard artist={artist} userWells={userWells} key={artist.id}/>)}</div>
        </div>
        </>
    )
} else {
    return (
        <>
        <div>
        <div className="empty-search">These are not the droids you are seaching for</div>
        </div>
        </>
    )
}
